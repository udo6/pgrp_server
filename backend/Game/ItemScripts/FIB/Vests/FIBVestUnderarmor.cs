...
namespace Game.ItemScripts.FIB.Vests
{
    public class FIBVestUnderarmor : ItemScript
	{
		private static readonly int ArmorDrawable = 0;
		private static readonly int ArmorTexture = 0;
		private static readonly uint ArmorDlc = 0;

		public FIBVestUnderarmor() : base(164, true)
		{
		}
		...
		public override void OnUse(RPPlayer player, InventoryModel inventory, InventoryItemModel item, int slot, int amount)
		{
			...
			player.StartInteraction(() =>
			{
				...
				player.SetArmor((ushort)(attribute == null ? 80 : attribute.Value));
